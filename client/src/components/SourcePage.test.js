...
let currentTopic, source, topicData, changeCurrentTopic, wrapper;
...
describe('SourcePage rendering', () => {
  ...
  it("renders the Topics component once", () => {
    expect(wrapper.find(Topics).length).toEqual(1);
  });

  it("renders the NewsFeed component once", () => {
    expect(wrapper.find(NewsFeed).length).toEqual(1);
  });
});