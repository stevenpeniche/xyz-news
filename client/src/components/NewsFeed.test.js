...
let currentTopic, source, topicData, wrapper;
...
beforeAll(() => {
  ...
  wrapper = shallow( <NewsFeed topicData={topicData} />);
});
...
describe('NewsFeed rendering', () => {
  ...
  it("renders the ResponsiveMasonry component once", () => {
    expect(wrapper.find(ResponsiveMasonry).length).toEqual(1);
  });

  it("renders the Masonry component once", () => {
    expect(wrapper.find(Masonry).length).toEqual(1);
  });

  it("renders the correct number of news-feed-card elements", () => {
    expect(wrapper.find('.news-feed-card').length).toEqual(topicData.length);
  });
});